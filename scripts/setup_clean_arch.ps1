...
Write-Step 'Adding runtime dependencies'
$deps = @(
  'get_it',
  'fpdart',
  'dio',
...
foreach ($d in $deps) { & dart pub add $d }
...
foreach ($d in $devDeps) { & dart pub add -d $d }
...
Write-Step 'Writing feature: Todo (presentation)'
@"
import 'package:equatable/equatable.dart';
...
Write-Ok 'Clean Architecture (MVVM + BLoC + Dio) scaffolded.'
